...
const deleteReward = async (filter) =>{
  try {
    let reward = await dbService.findAll(Reward,filter);
    if (reward && reward.length){
      reward = reward.map((obj) => obj.id);

      const OrderFilter = { $or: [{ rewardId : { $in : reward } }] };
      const OrderCnt = await dbService.destroy(Order,OrderFilter);

      let deleted  = await dbService.destroy(Reward,filter);
      let response = { Order :OrderCnt.length, };
      return response; 
    } else {
      return {  reward : 0 };
    }
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const RewardFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const RewardCnt = await dbService.destroy(Reward,RewardFilter);

      const OrderFilter = { $or: [{ userId : { $in : user } },{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const OrderCnt = await dbService.destroy(Order,OrderFilter);
...
const softDeleteReward = async (filter,updateBody) =>{  
  try {
    let reward = await dbService.findAll(Reward,filter, { id:1 });
    if (reward.length){
      reward = reward.map((obj) => obj.id);

      const OrderFilter = { '$or': [{ rewardId : { '$in' : reward } }] };
      const OrderCnt = await dbService.update(Order,OrderFilter,updateBody);
      let updated = await dbService.update(Reward,filter,updateBody);
